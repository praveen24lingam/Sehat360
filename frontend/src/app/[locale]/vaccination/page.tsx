...
export default function VaccinationPage() {
  ...
  const VaccineGroup = ({ title, items, defaultOpen = true }: { title: string, items: typeof vaccineStatuses, defaultOpen?: boolean }) => {
    ...
    return (
      <div className="mb-5">
        <div className="flex items-center justify-between mb-2.5 cursor-pointer select-none" onClick={() => setOpen(!open)}>
          <div className="flex items-center gap-2">
            <p className="section-label">{title}</p>
            <span className="bg-brand-smoke text-brand-inkSoft text-[10px] font-bold px-1.5 py-0.5 rounded-full">{items.length}</span>
          </div>
          <span className="text-xs font-semibold text-brand-deepGreen">{open ? '↑ Chhupayein' : '↓ Dekho'}</span>
        </div>

        <AnimatePresence>
          {open && (
            <motion.div variants={ANIM_VARIANTS} initial="hidden" animate="visible" exit="exit" className="flex flex-col gap-2.5 overflow-hidden">
              {items.map(v => (
                <div key={v.id} className="bg-white border border-brand-border rounded-2xl p-4 shadow-card flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${v.status === 'overdue' ? 'bg-brand-danger' : v.status === 'dueSoon' ? 'bg-brand-saffron' : v.status === 'done' ? 'bg-brand-midGreen' : 'bg-brand-border'}`} />
                    <div>
                      <h4 className="font-bold text-sm text-brand-ink leading-tight">{v.name}</h4>
                      <p className="text-[11px] text-brand-inkSoft mt-0.5">{t(v.dueLabelKey as any)} · {formatDateInLocale(v.dueDate, language)}</p>
                    </div>
                  </div>

                  <div className="flex flex-col items-end gap-1.5 shrink-0 ml-3">
                    {v.status === 'overdue' && <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${getStatusStyles('overdue')}`}>{t('vaccination.status.overdue')}</span>}
                    {v.status === 'dueSoon' && <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${getStatusStyles('dueSoon')}`}>{t('vaccination.status.dueSoon')}</span>}
                    {v.status === 'done' && <span className={`text-[11px] font-semibold ${getStatusStyles('done')}`}>✓ {formatDateInLocale(v.doneDate!, language)}</span>}
                    {(v.status === 'overdue' || v.status === 'dueSoon' || v.status === 'upcoming') && (
                      <button onClick={() => setMarkDoneId(v.id)} className="text-[11px] font-bold text-brand-deepGreen">{t('vaccination.markDone')}</button>
                    )}
                  </div>
                </div>
              ))}
    ...
    <div className="bg-gradient-to-br from-brand-deepGreen to-brand-midGreen rounded-3xl p-5 text-white shadow-card-md relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 rounded-2xl bg-white/20 flex items-center justify-center font-bold text-sm shrink-0">{initials}</div>
          <div>
            <h3 className="font-bold text-sm leading-tight">{selectedChild.name}</h3>
            <p className="text-xs text-white/70 mt-0.5">{ageMonths} {t('common.months')} • {t('vaccination.title')}</p>
          </div>
        </div>
        <div className="flex items-end justify-between mb-2">
          <div>
            <p className="text-xs text-white/60 font-medium mb-1">{t('vaccination.progress', { percent: '' }).replace('%', '')}</p>
            <div className="flex items-baseline gap-1">
              <span className="font-mono font-black text-5xl leading-none">{percent}</span>
              <span className="font-bold text-xl text-white/80">%</span>
            </div>
          </div>
          <div className="text-right">
            <p className="text-white/60 text-[11px] font-medium">{doneCount} / {totalCount}</p>
            <p className="text-white/60 text-[10px]">{t('vaccination.group.done')}</p>
          </div>
        </div>
        <div className="w-full bg-white/20 rounded-full h-1.5 overflow-hidden mb-4"><div className="bg-white h-full rounded-full transition-all duration-500" style={{ width: `${percent}%` }} /></div>
        <div className="flex flex-wrap gap-2">
          {overdue.length > 0 && <span className="bg-white/15 text-white rounded-full px-2.5 py-1 text-[10px] font-semibold">{overdue.length} {t('vaccination.group.overdue')}</span>}
          {dueSoon.length > 0 && <span className="bg-white/15 text-white rounded-full px-2.5 py-1 text-[10px] font-semibold">{dueSoon.length} {t('vaccination.group.dueSoon')}</span>}
          <span className="bg-white/15 text-white rounded-full px-2.5 py-1 text-[10px] font-semibold">{doneCount} {t('vaccination.group.done')}</span>
        </div>
      </div>
    </div>
  )
      ...
      <div className="p-4 flex flex-col gap-5">
        ...
        <section>
          <div className="bg-white border border-brand-border rounded-2xl p-4 shadow-card flex items-center gap-4">
            <div className="w-11 h-11 rounded-2xl bg-brand-blueLight flex items-center justify-center shrink-0"><MapPin size={20} className="text-brand-blue" /></div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-brand-ink leading-tight mb-0.5">Free vaccines najdeeki PHC mein</p>
              <p className="text-xs text-brand-inkSoft mb-2.5">Sarkari hospital mein sabhi vaccines free milti hain</p>
              <Button variant="outline" size="sm" onClick={handlePHC} className="h-8 text-xs border-brand-border rounded-xl px-3"><MapPin size={11} className="mr-1.5" /> Map kholein</Button>
            </div>
          </div>
        </section>
      </div>
